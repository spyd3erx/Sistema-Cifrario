...
def crear_usuario_supervisor(sender, instance, created, **kwargs):
    if created and not instance.user_id:
        ...
        Supervisor.objects.filter(id=instance.id).update(user=user)

        # Intentar enviar correo de credenciales
        try:
            send_mail = SendMailCredenciales(instance=instance,
                                             asunto="Tu cuenta de Supervisor en el sistema de horas sociales - Cifrario",
                                             username=username,
                                             password=password)
